...
import { getLeaderboard } from '../utils/flipApi';
...
const LeaderboardTooltip = ({ isOpen, panelRef, maxHeight }) => {
  ...
  useWatchContractEvent({
    ...
    enabled: isOpen, // Only enable when leaderboard is visible
    onLogs: (logs) => {
      logs.forEach((log) => {
        try {
    ...
    onLogs: (logs) => {
      logs.forEach((log) => {
        try {
          ...
          const betMon = parseFloat(formatEther(amount));
          const points = betMon; // 1 MON = 1 point
          const pnlChange = winner ? (betMon * 0.95) : -betMon;
          ...
          setLeaderboard(prev => {
            ...
            if (existingIndex >= 0) {
              ...
              // Yeni oyuncu ekle (listede yoksa)
              updated.push({
                player: player,
  ...
  const getPlayerDisplay = (player) => {
    if (player.discord?.username) {
      const name = player.discord.globalName || player.discord.username;
