...
import { getLeaderboard } from '../utils/hiloApi';
...
const LeaderboardTooltip = ({ isOpen, panelRef, maxHeight }) => {
  ...
  // BetPlaced event'ini dinle - amount bilgisini kaydet
    ...
    onLogs: (logs) => {
      logs.forEach((log) => {
        try {
          ...
          const pnlChange = winner ? (payoutMon - betMon) : -betMon;
